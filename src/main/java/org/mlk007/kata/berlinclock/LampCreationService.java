...
public class LampCreationService {
	...
	public String getSecondRowForHours(int hour) {
		...
		return getHoursRowsString(switchedOnLamps, HOUR_ROW_LENGTH,
				LampColourEnum.RED);
	}
	...
	public String getFourthRowForMinutes(int minute) {
		...
		return getMinutesFirstRowString(switchedOnLamps,
				MINUTE_FIRST_ROW_LENGTH);
	}
	...
	public String getFifthRowForMinutes(int minutes) {
		...
		return getHoursRowsString(switchedOnLamps, MINUTE_SECOND_ROW_LENGTH,
				LampColourEnum.YELLOW);
	}

	private void validateHourValue(int hour) {
		if (hour < 0 || hour > 24)
			throw new IllegalArgumentException(
					"Value of hour should be between 0 and 24");
	}

	private void validateMinuteValue(int minute) {
		if (minute < 0 || minute > 60)
			throw new IllegalArgumentException(
					"Value of minute should be between 0 and 60");
	}

	private void validateSecondsValue(int second) {
		if (second < 0 || second > 60)
			throw new IllegalArgumentException(
					"Value of seconds should be between 0 and 60");
	}

	private String getHoursRowsString(int switchedOnLamps, int lampCount,
			LampColourEnum lampColourEnum) {
